...
abstract class Command implements ICommand {
	...
	public ModifyPermissions(type: string, action: string, key: string, local = false) {
		...
		if (!local) {
			...
			if (action === 'add') {
				...
				this.LocalData[permissionKey] = this.LocalData[permissionKey].filter(
					(entry) => key === entry
				);
			}
		} else {
	...
	public Call(message: Message, isOwner: boolean): Promise<any> {
		if (!isOwner) {
			...
			if (
				!this.ValidateRoles(message.member.roles) ||
				!this.ValidateChannel(message.channel.id)
			) {
				console.warn(
					'[Failed Permission]',
					message.member.displayName,
				...
				message.channel.send(
					`[Permission Failure] I'm afraid I can't let you do that, ${message.member
						.displayName}`
				);
			...
			if (isOnCooldown) {
				message.channel.send(
					'[Rate Warning] This command is still on cooldown, please wait.'
				);
			}
		}
